...
|-----------|-------------|----------|
| Sidebar nav | `AppNavigation` | `getByRole('button', { name: '📊 Dashboard' })`, `… '🎓 Programs'` |
...
| `Edit Program` | `EditProgramModal` | `getByRole('dialog', { name: 'Edit Program' })`, same fields, `Save` instead of `Create`, `Cancel` |

Programs list table:

- Row targeting: `getByRole('row').filter({ hasText: programName })`
- Row actions: `row.getByRole('button', { name: '✏️' })`, `row.getByRole('button', { name: '🗑' })`
